...
**Latest commits this session (most recent on top), on branch `claude/explore-mobile-handoff-0yWca`:**
...
- [x] Migrations through `documents/0021_document_download_disclaimer` (accounts through `0006_user_is_tester`, public_pages through `0002_newsitem`)
...
- **Free-access promo code for testers:** new management command `python manage.py seed_tester_promo` (in `documents/management/commands/`). Creates (or refreshes) a `PromoCode` with `discount_type='free'`, `is_active=True`. Defaults the code to `TESTFREE-<random6>` so it can't be guessed by random visitors; override with `--code MYCODE`. Run with `--deactivate --code MYCODE` to flip `is_active=False` when the round ends. Run on Render Shell when starting a testing round; copy the printed code into your tester instructions.
