...
**NEW FEATURE (this session): Citizen Complaint Assistant — LIVE and confirmed working on production.** Built on branch `claude/citizen-complaint-assistant-9ym1be` (merged to `master`, deployed, all 4 new API keys — `YOUTUBE_API_KEY`, `SUPADATA_API_KEY`, `SERPAPI_API_KEY`, plus the pre-existing `OPENAI_API_KEY` — are set on Render). Free (no payment gate), login-required, entirely new Django app `citizen_complaint`. Full flow tested end-to-end on production including a real send. See its dedicated section below for the complete breakdown; the short version of what shipped across this session's iterations:
- Full 5-step wizard (video intake → agencies → about-you → drafts → send), a "My Complaints" list page (`/citizen-complaint/mine/`, nav-linked) to resume or re-download later, terminal-style progress screens on the two slow handoffs
- Email verification added to `accounts.User` (gates only the send step) — admin now has a manual override (Email Verification fieldset + bulk action) for accounts that never went through the register view (e.g. `createsuperuser`)
- Agency-email lookup uses SerpApi, **not** Google Custom Search — Google blocks "search the entire web" for new engines as of Jan 2026, Bing Search API is fully retired
- A fail-closed OpenAI Moderation gate on every send, narrowed to actual-threat categories (not OpenAI's raw `flagged`, which trips on any description of violence — a real bug caught in production testing that would have blocked nearly every legitimate complaint)
...
- 5 tests (`DescriptionContactExtractionTest`) using the actual UC Davis-style description block as the fixture, covering: regex-verification drops hallucinated emails, one row per named contact gets created and pre-checked, curated-DB-wins-alongside-named-contacts, and the name-mismatch regression below.
- **Bug fix, same session:** first production test showed the real UC Davis contact info never appearing — only unrelated AI-guessed agencies ("City of Davis", "UC Davis"). Root cause: `agency_contacts` is keyed by whatever agency name the AI wrote in that part of its response, which isn't guaranteed to match how the same agency is spelled in the plain `agencies` list (it had collapsed "U.C. Davis Police Department" / "U.C. Davis Administration" down to generic "City of Davis" / "UC Davis" there). The original matching-by-name logic silently dropped the contacts whenever the two didn't line up. Fixed by processing each `agency_contacts` entry using its own name directly — no matching against the `agencies` list required.

**⚠️ CITIZEN COMPLAINT ASSISTANT IS CURRENTLY SWITCHED OFF FOR USERS — and why.** Production testing of the description-contact feature above surfaced a blocker that has nothing to do with this app's code: **the YouTube Data API "Queries per day" quota on the Google Cloud project is provisioned at 0**, so every metadata call 403s ("exceeded your quota") and the oEmbed fallback 401s on top of it. No metadata means no video description, which means the whole description-contact feature — and the AI extraction generally — has nothing good to work from and falls back to generic agency guesses. Suspected cause: **no billing account linked to the Google Cloud project** (Google won't provision the standard free-tier YouTube quota without one). Verify at `https://console.cloud.google.com/apis/api/youtube.googleapis.com/quotas` and the Billing page for that project. Note two API keys were rotated during this debugging (both were pasted in plaintext in chat and retired).
Until that's fixed, the feature is hidden behind a new admin switch rather than removed:
- **`SiteSettings.citizen_complaint_enabled`** (migration `accounts/0008_sitesettings_citizen_complaint_enabled`) — a Feature Flags checkbox at `/<ADMIN_URL>/accounts/sitesettings/1/change/`. The model default is `True` (a working feature's normal state, and what the test suite gets), but that migration's `RunPython` step **switches it off for this deployment on the way in**, so merging + deploying is all it takes to hide it.
- **Off means off, not just hidden links:** the two navbar links disappear, AND `citizen_complaint.views.feature_enabled` (a decorator on all 9 views) raises `Http404` — which this project's `handler404 = redirect_to_home` turns into a redirect to the homepage, same as any unknown URL. So bookmarked/shared `/citizen-complaint/` URLs stop working too, including mid-wizard step URLs.
- **Staff are exempt** — `is_staff` accounts still reach the whole feature while it's off (so you can re-test once the quota is fixed), and see a small amber "off" badge on the navbar link as a reminder it's hidden from everyone else.
- **Nothing is deleted.** Every `Incident` / `TargetAgency` / `Complaint` row stays untouched; re-checking the box restores the feature exactly as it was. A `FeatureSwitchTest` class covers all of this (including that disabling preserves existing rows).
...
- `0007_user_email_verified_user_email_verified_at` — adds `User.email_verified` / `email_verified_at` (gates only the Citizen Complaint send step)
- `0008_sitesettings_citizen_complaint_enabled` — adds `SiteSettings.citizen_complaint_enabled` (model default `True`) **plus a `RunPython` step that switches it off for this deployment**, parking the Citizen Complaint Assistant while its YouTube Data API quota problem is sorted out. See the warning block near the top of this doc.
